...
long <- clean %>%
  pivot_longer(
    ...
    names_pattern = "^(BN_vs_HC|BN_vs_recBN|recBN_vs_HC)_(beta|SE|P|PBonferroni)$"
  ) %>%
  mutate(
    comparison = recode(comparison,
                        BN_vs_HC = "BN vs HC",
                        BN_vs_recBN = "BN vs recBN",
                        recBN_vs_HC = "recBN vs HC"),
    comparison = factor(comparison, levels = c("BN vs HC","BN vs recBN","recBN vs HC")),
    ci_low  = beta - 1.96 * SE,
...
## =========================
## 3) Order measures WITHIN each domain (by BN vs HC beta)
## =========================
...
print(p)
ggsave("BN_comparisons_by_domain.pdf", p, width = 16, height = 9, dpi = 300)
