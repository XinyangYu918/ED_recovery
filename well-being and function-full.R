...
# Load data
df <- read_excel("for rec GMV analysis-2.xlsx", sheet = 8)

# Recode variables
df[, c("HF_02", "HF_03", "HF_04","HF_05", "HF_06")] <- lapply(df[, c("HF_02", "HF_03", "HF_04","HF_05", "HF_06")], function(x) {
  x[x == 88] <- 0
  x[x %in% c(77, 99)] <- NA
  return(x)
})

df[, paste0("HF_", sprintf("%02d", 7:18))] <- lapply(df[, paste0("HF_", sprintf("%02d", 7:18))], function(x) {
  x[x == 9] <- NA
  return(x)
})
...
df[, 6:28] <- lapply(df[, 6:28], as.numeric)
df[, 6:28] <- lapply(df[, 6:28], scale)
...
roi_names <- colnames(df_filtered)[c(6:28)]
...
combined_results$ROI <- recode(combined_results$ROI,
                               WHO5_sum = "Well-Being Index",
                               PANAS_PAS = "Positive affect",
                               PANAS_NAS = "Negative affect",
                               PSS_sum = "Perceived stress",
                               HF_01 = "General health rating",
                               HF_02 = "Days physical health not good (past 30 days)",
                               HF_03 = "Days mental health not good (past 30 days)",
                               HF_04 = "Days health kept from usual activities (past 30 days)",
                               HF_05 = "Physical activity in past month (excluding job)",
                               HF_06 = "Average hours of sleep per 24-hour period",
                               HF_07 = "Sleep quality",
                               HF_08 = "Ever told had heart attack",
                               HF_09 = "Ever told had angina or coronary heart disease",
                               HF_10 = "Ever told had stroke",
                               HF_11 = "Ever told had asthma",
                               HF_12 = "Ever told had skin cancer",
                               HF_13 = "Ever told had other types of cancer",
                               HF_14 = "Ever told had COPD, emphysema, or chronic bronchitis",
                               HF_15 = "Ever told had arthritis, gout, lupus, or fibromyalgia",
                               HF_16 = "Ever told had depressive disorder",
                               HF_17 = "Ever told had kidney disease",
                               HF_18 = "Ever told had diabetes")
...
p1 <- combined_results %>%
  ...
  labs(title = "", y = "Effect size (standardised)", x = "Well-being and function", color = "Significance (Bonferroni-corrected)") +
...
ggsave("well being_and_function_full_comparisons.pdf", width = 12, height = 10, dpi = 300)
