...
df[, 7:47] <- lapply(df[, 7:47], as.numeric)
df[, 7:47] <- lapply(df[, 7:47], scale)
df$internalising <- df$semotion + df$speer
df$externalising <- df$sconduct + df$shyper
...
roi_names <- colnames(df_filtered)[c(19:21, 31:44,48:49)]
...
combined_results$ROI <- recode(combined_results$ROI,
                               ...
                               semotion = "Emotional symptoms",
                               sconduct = "Conduct disorders",
                               shyper = "Hyperactivity/inattention",
                               speer = "Peer relationship problems",
                               sprosoc = "Prosocial behaviours",
                               internalising = "Internalising problems",
                               externalising = "Externalising problems",
                               BSI_Somatization = "Somatisation",
                               BSI_Obsession_Compulsion = "Obsession-compulsion",
                               BSI_Interpersonal_Sensitivity = "Interpersonality sensitivity",       
                               BSI_Depression = "Depression",
                               BSI_Anxiety = "Anxiety",
                               BSI_Hostility = "Hostility",
                               BSI_Phobic_Anxiety = "Phobic anxiety",
                               BSI_Paranoid_Ideation = "Paranoid ideation",
                               BSI_Psychoticism = "Psychoticism",
                               BSI_Positive.symptom.total = "Positive symptom total",
                               BSI_Positive.symptom.distress.index = "Positive symptom distress index",
                               BSI_General.severity.index = "General severity index")
...
p1 <- combined_results %>%
  ...
  labs(title = "", y = "Effect size (standardised)", x = "Eating behaviours and mental health", color = "Significance (Bonferroni-adjusted)") +
...
ggsave("mental_health_full_comparisons.pdf", width = 10, height = 10, dpi = 300)
